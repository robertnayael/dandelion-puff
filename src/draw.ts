import { offscreenCanvas } from './main';
...
export default function draw(state: State, ctx: CanvasRenderingContext2D) {
  ...
  if (showDebugOverlay) {
    state.vectorField.forEach(({ vector, row, column, index, cellSize }) => {
      const cell = {
        x: column * cellSize,
        y: row * cellSize,
      };
  ...
  // primaryCtx.drawImage(offscreenCanvas, 0, 0, offscreenCanvas.width / devicePixelRatio, offscreenCanvas.height / devicePixelRatio);
}
